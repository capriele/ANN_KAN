        ...
        x0 = model.model.bridgeNetwork(
            torch.cat(
                [
                    ...
                    torch.tensor(x0.T, dtype=torch.float32),
                ]
            ).T  ## Need to transpose (7x1) to (1x7) for the model (matrix multiplication)
        )
        y = model.model.decoderNetwork(x0)
        logY += [y]
        logX += [x0]
...
def costFunction(uSequence, r, um1, logAB, logC, x0):
    ...
    for u in uSequence:
        ...
        if isinstance(logC[i], torch.Tensor):
            logC[i] = logC[i].detach().cpu().numpy()
        x0 = np.dot(logAB[i][0], asda[0 : Option.stateSize])
        x0 = x0 + np.reshape(logAB[i][0], (1, Option.stateSize))
        y = np.dot(logC[i][0], x0.T)
        logY += [y[0][-1]]
        i = i + 1
    #    logY+=[y[0][1]]
    logY = np.array(logY)
...
def evaluateFeatureImportance():
    ...
    if not Option.stateReduction:
        w = model.model.encoderNetwork.get_layer("enc00").get_weights()
        ...
        w = model.model.decoderNetwork.get_layer("dec00").get_weights()
    ...
    for i in range(0, finalRange):
        ...
        if i < openLoopStartingPoint or (i % _reset == 0 and _reset > 0):
            ...
            x0 = model.model.bridgeNetwork(
                torch.cat(
                    [
                        torch.tensor(u, dtype=torch.float32).T,
                        torch.tensor(x0, dtype=torch.float32).T,
                    ]
                ).T
            )
        y = model.model.decoderNetwork(x0)[0]
...
for r in reset:
    for voM in validationOnMultiHarmonic:
        ...
        fit, NRMSE, logY, logYR = openLoopValidation(
            validationOnMultiHarmonic=voM, _reset=r, YTrue=YtrueToPass, U_Vn=U_Vn.copy()
        )
        end = time.time()
...
scipy.io.matlab.savemat(
    ...
    {"U": U_n, "Y": Y_n, "U_val": U_Vn, "Y_val": Y_Vn, "Option": str(Option.__dict__)},
)
