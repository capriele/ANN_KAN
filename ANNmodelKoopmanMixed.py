...
class BridgeNetwork(nn.Module):
    """Bridge network that combines BridgeNetworkA and BridgeNetworkB outputs."""

    def __init__(
        self,
        bridge_network_a: BridgeNetworkA,
        bridge_network_b: BridgeNetworkB,
        alpha: float = 0.5,
    ):
        super().__init__()
        self.bridge_network_a = bridge_network_a
        self.bridge_network_b = bridge_network_b
        self.alpha = alpha
    ...
    def forward(self, inputs_novelU: torch.Tensor, inputs_state: torch.Tensor) -> Union[
        ...
        out_a = self.bridge_network_a(inputs_novelU, inputs_state)
        out_b = self.bridge_network_b(inputs_novelU, inputs_state)

        # Extract the main output (first element) from each network
        main_out_a = out_a[0]
        main_out_b = out_b[0]
        ...
        if len(out_a) > 1 and len(out_b) > 1:
            return (combined_out, *out_a[1:], *out_b[1:])
        else:
            return (combined_out,)
