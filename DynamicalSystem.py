...
class LinearSystem:
    ...
    def systemDynamics(self,dim, flag=True,U=-1):
        ...
        # u_n=0
        noise = np.random.normal(0, 1, size=(self.inputSize, dim))
        if  flag:            
            print('a')
        else:
            print('b')
        ...
        for i in range(0,dim):        
            ...
            x_k = self.stateMap(x_k, np.reshape(uSat, (self.inputSize, 1))) * 1
    ...
    def prepareDataset(self,sizeT,sizeV):
        ...
        print(u_Vn.shape)
        return (
            np.reshape(u_n, (sizeT, 1)),
            np.reshape(y_n, (sizeT, 1)),
            np.reshape(u_Vn, (sizeV, 1)),
            np.reshape(y_Vn, (sizeV, 1)),
        )
